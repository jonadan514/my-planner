...
import { hasNativeHealthConnectBridge, syncHealthConnect, syncHealthConnectIfStale } from '../integrations/healthConnect'
...
const VISIBLE_DATA_TYPES: HealthDataType[] = ['EXERCISE', 'STEPS', 'SLEEP', 'WEIGHT', 'BODY_FAT']
...
export default function HealthConnectCard() {
  ...
  useEffect(() => {
    syncHealthConnectIfStale().then(setState).catch(error => setState({
      status: 'ERROR',
      grantedDataTypes: [],
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-4">
      <div className="flex items-center justify-between mb-2"><p className="text-sm font-semibold text-gray-900">Samsung Health · Health Connect</p><button onClick={sync} disabled={busy} className="text-xs text-emerald-500 disabled:text-gray-300">{busy ? '동기화 중' : state.status === 'PERMISSION_REQUIRED' ? '건강 권한 연결' : state.status === 'PARTIAL' ? '추가 권한 연결' : '지금 동기화'}</button></div>
      ...
      <p className="mt-2 text-[10px] leading-4 text-gray-500">운동 목록에는 삼성헬스의 러닝과 웨이트 기구운동만 구분해 가져옵니다.</p>
      ...
      {state.lastSuccessfulSyncAt && (
        <p className={`mt-1 text-[10px] ${state.lastRecordCount ? 'text-emerald-600' : 'text-amber-500'}`}>
          최근 30일 건강기록 {state.lastRecordCount ?? 0}건 확인
          {state.sourcePackages?.length ? ` · ${state.sourcePackages.join(', ')}` : ''}
        </p>
      )}
      {state.lastSuccessfulSyncAt && state.lastRecordCount === 0 && (
        <p className="mt-1 text-[10px] leading-4 text-gray-400">
          Samsung Health → 설정 → Health Connect에서 건강 데이터 공유가 켜져 있는지 확인해 주세요.
        </p>
      )}
      {state.status === 'PARTIAL' && <p className="mt-1 text-[10px] text-amber-500">허용되지 않은 항목이 있습니다. 필요한 항목만 추가로 연결할 수 있습니다.</p>}
      {state.status === 'ERROR' && <p className="text-[10px] text-red-400 mt-1">{state.errorCode || '동기화 중 오류가 발생했습니다.'}</p>}
    </div>
  )
}
