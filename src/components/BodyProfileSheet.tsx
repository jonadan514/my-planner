      ...
      <div className="modal-sheet max-h-[90dvh] w-full max-w-[480px] overflow-y-auto rounded-t-3xl bg-white p-5" onClick={event => event.stopPropagation()}>
        ...
        {recommendation ? (
          <div className="mt-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
            <p className="text-xs font-semibold text-emerald-800">추천 목표 · 기준체중 {recommendation.referenceWeightKg}kg</p>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          ...
          <button type="button" onClick={() => void save(true)} className="rounded-xl bg-emerald-500 py-3 text-sm font-semibold text-white">추천 목표 적용</button>
        </div>
      </div>
    </div>
  )
